...
input_tensor_x = np.empty([7, 50030])
input_tensor_y = np.empty([7, 50030])
...
input_tensor_x[0] = atoms10['L_Raw_X'][:50030]
input_tensor_x[1] = atoms14['L_Raw_X'][:50030]
input_tensor_x[2] = atoms23['L_Raw_X'][:50030]
input_tensor_x[3] = atoms26['L_Raw_X'][:50030]
input_tensor_x[4] = atoms4['L_Raw_X'][:50030]
input_tensor_x[5] = atoms9['L_Raw_X'][:50030]
input_tensor_x[6] = atoms18['L_Raw_X'][:50030]

input_tensor_y[0] = atoms10['L_Raw_Y'][:50030]
input_tensor_y[1] = atoms14['L_Raw_Y'][:50030]
input_tensor_y[2] = atoms23['L_Raw_Y'][:50030]
input_tensor_y[3] = atoms26['L_Raw_Y'][:50030]
input_tensor_y[4] = atoms4['L_Raw_Y'][:50030]
input_tensor_y[5] = atoms9['L_Raw_Y'][:50030]
input_tensor_y[6] = atoms18['L_Raw_Y'][:50030]

atoms28 = pd.read_csv('atoms28 pre-processed.csv')
atoms38 = pd.read_csv('atoms38 pre-processed.csv')
# atoms33 = pd.read_csv('atoms33 pre-processed.csv')

test_tensor_x = np.zeros([4, 50030])
test_tensor_y = np.zeros([4, 50030])

test_tensor_x[0] = atoms28['L_Raw_X'][:50030]
test_tensor_x[1] = atoms38['L_Raw_X'][:50030]
test_tensor_x[2] = atoms33['L_Raw_X'][:50030]
test_tensor_x[3] = atoms49['L_Raw_X'][:50030]

test_tensor_y[0] = atoms10['L_Raw_Y'][:50030]
test_tensor_y[1] = atoms14['L_Raw_Y'][:50030]
test_tensor_y[2] = atoms33['L_Raw_Y'][:50030]
input_tensor_y[3] = atoms49['L_Raw_Y'][:50030]
...
trainX_x, trainY_x = create_dataset(input_tensor_x, 30)
trainX_y, trainY_y = create_dataset(input_tensor_y, 30)
...
testX_x, testY_x = create_dataset(test_tensor_x, 30)
testX_y, testY_y = create_dataset(test_tensor_y, 30)
...
def construct_stateful(hidden_size, layers, lrate, opt, window_size):
    ...
    for i in range(layers):
        model.add(LSTM(hidden_size, batch_input_shape=(4, window_size, 2), return_sequences=True, dropout_W=0.2,
                       stateful=True))
        ...
        if (i > 0):
            ...
            for j in range(len(x)):
                ...
                if current_value[0] == 0:
                    ...
                    euclid_distance += (current_pred[0] - current_answer[0]) ** 2 + (current_pred[1] - current_answer[
                        1]) ** 2
        '''try:
            acc = float(current_student_correct) / (current_student_incorrect + current_student_correct)
            per_student_accuracies.append(acc)
            ...
            if (i > 0):
                ...
                for j in range(len(x)):
                    ...
                    if current_value[0] == 0:
                        ...
                        current_pred = list(naive_prediction[j])
                        euclid_distance += (current_pred[0] - current_answer[0]) ** 2 + (current_pred[1] -
                                                                                         current_answer[1]) ** 2
        '''try:
            acc = float(current_student_correct) / (current_student_incorrect + current_student_correct)
            per_student_accuracies.append(acc)
...
results30timestep = 'eye_tracking_results_30timestep_march5.csv'

per_timestep_30 = '30timestep_exp3.csv'

entries = []
for j in range(1):
    window_size = int(train2d.shape[2] / 10)
    model = construct_chopped_nonstateful(100, 1, 0.01, RMSprop, window_size)
    chopped_1_100, vals_chopped_1_100 = train_chopped_model(model, train2d.shape[2], 10, train2d, labels2d, test2d,
                                                            test_labels2d)
    print(chopped_1_100)
    ...
    stateful_1_100, vals_stateful_1_100 = train_stateful(stateful_model, 10, 4, train2d.shape[2], train2d, labels2d,
                                                         test2d, test_labels2d, shuffle=True)
    print("1 layer 100 hidden nodes")
    ...
    chopped_2_200, vals_chopped_2_200 = train_chopped_model(model, train2d.shape[2], 10, train2d, labels2d, test2d,
                                                            test_labels2d)
    ...
    stateful_2_200, vals_stateful_2_200 = train_stateful(stateful_model, 10, 4, train2d.shape[2], train2d, labels2d,
                                                         test2d, test_labels2d, shuffle=True)
    ...
    if (j == 0):
        with open(per_timestep_30, 'w') as csvfile:
            resultswriter = csv.writer(csvfile)
            resultswriter.writerow(
                ['vals_chopped_1_100', 'vals_unchopped_1_100', 'vals_stateful_1_100', 'vals_chopped_2_200',
                 'vals_stateful_2_200'])
        ...
        with open(results30timestep, 'w') as csvfile:
            resultswriter = csv.writer(csvfile)
            resultswriter.writerow([chopped_1_100, unchopped_1_100, stateful_1_100, chopped_2_200, stateful_2_200])
